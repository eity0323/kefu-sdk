...
import com.easemob.cec.Preferences;
import com.easemob.cec.R;
import com.easemob.cec.receiver.FloatWindow;
import com.easemob.cec.widget.CallControllers;
import com.easemob.cec.widget.CircleImageView;
import com.easemob.cec.widget.MyChronometer;
...
public class CallActivity extends DemoBaseActivity implements CallManager.CallManagerDelegate {
	...
	private void initViews(){
		...
		ib_minimize = (ImageView) findViewById(R.id.ib_minimize);
		mIvHide.setVisibility(View.INVISIBLE);
		ib_minimize.setVisibility(View.INVISIBLE);
			...
			public void onClick(View v) {
				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
					...
					if (!Settings.canDrawOverlays(CallActivity.this)) {
						...
						// 开启悬浮窗
						FloatWindow.getInstance(CallActivity.this).addFloatWindow();
					}
				}
			}
		});
	...
	private void showAlertDialog() {
		...
		dialogFragment.setTitleText("悬浮窗权限");
		dialogFragment.setContentText(getString(R.string.to_setting));
		dialogFragment.setupRightBtn("去开启", new View.OnClickListener() {
			@Override
			public void onClick(View v) {
...
//		Glide.with(this).load(R.drawable.hd_default_image).transform(new GlideCircleTransform(this)).into(imageView);
	...
	public void onNotice(CallManager.HMediaNoticeCode code, String arg1, String arg2, Object arg3) {
		if (code == CallManager.HMediaNoticeCode.HMediaNoticeOpenCameraFail) {
			EMLog.e(TAG, "onNotice:HMediaNoticeOpenCameraFail");
		} else if (code == CallManager.HMediaNoticeCode.HMediaNoticeOpenMicFail) {
			EMLog.e(TAG, "onNotice:HMediaNoticeOpenMicFail");
		} else if (code == CallManager.HMediaNoticeCode.HMediaNoticeTakeCameraPictureFailed) {
